...
def _admin_unauthorized():
    raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail='Authentication required', headers={'WWW-Authenticate': 'Basic'})
...
def require_current_dashboard(request: Request):
    if not _uses_current_money_scale(request):
        raise HTTPException(
            status.HTTP_409_CONFLICT,
            'Страница биллинга устарела. Обновите вкладку и повторите действие.',
        )
    return True

ADMIN_WRITE_AUTH = ADMIN_AUTH + [Depends(require_current_dashboard)]
...
class FinalizeIn(BaseModel):
    ...
    sell_billing_cycle: str = db.DEFAULT_BILLING_CYCLE
    cost_billing_cycle: str = db.DEFAULT_BILLING_CYCLE
...
def reserve(data: ReserveIn):
    ...
    provider_number = ''
    max_seconds = None
    sell_billing_cycle = db.DEFAULT_BILLING_CYCLE
    cost_billing_cycle = db.DEFAULT_BILLING_CYCLE
    try:
        now_ts = db.now()
        ...
        stage = 'client_rate'
        route_seed = data.call_uuid or data.sip_call_id or f"{data.sip_ip}:{data.sip_port}:{data.clid}:{data.destination}:{now_ts}"
        ...
        cost_billing_cycle = db.normalize_billing_cycle(_row_value(route, 'billing_cycle', db.default_billing_cycle_for_route(route['destination_name'], route['prefix'])))
        group = None
        ...
        return {'allowed': True, 'max_seconds': max_seconds, 'sell_rate_cents': rate['sell_rate_cents'], 'cost_rate_cents': route['cost_rate_cents'], 'sell_billing_cycle': sell_billing_cycle, 'cost_billing_cycle': cost_billing_cycle, 'gateway_name': gateway_name, 'route_ip': route_ip, 'tech_prefix': route['tech_prefix'], 'client_tech_prefix': client_tech_prefix, 'dial_destination': dial_destination, 'provider_number': provider_number, 'terminator_id': route['id'], 'terminator_name': route['name'], 'terminator_destination_name': route['destination_name'], 'terminator_prefix': route['prefix'], 'terminator_tech_prefix': route['tech_prefix'], 'client_id': client['id'], 'call_uuid': call_uuid}
    except HTTPException as exc:
        conn.rollback()
        _safe_record_sip_hit(data, status_text='rejected', stage=stage, reason=str(exc.detail), client=client, rate=rate, route=route, gateway_name=gateway_name, route_ip=route_ip, dial_destination=dial_destination, provider_number=provider_number, client_tech_prefix=client_tech_prefix, max_seconds=max_seconds, sell_billing_cycle=sell_billing_cycle, cost_billing_cycle=cost_billing_cycle)
        raise
    finally:
        conn.close()
...
@app.post('/api/clients', dependencies=ADMIN_WRITE_AUTH)
...
@app.patch('/api/clients/{cid}', dependencies=ADMIN_WRITE_AUTH)
...
@app.post('/api/clients/{cid}/topup', dependencies=ADMIN_WRITE_AUTH)
def topup(cid: int, data: TopupIn):
    conn = db.get_conn()
    try:
        conn.execute('BEGIN IMMEDIATE')
        cur = conn.execute('UPDATE clients SET balance_cents = balance_cents + ? WHERE id = ?', (data.amount_cents, cid))
        if cur.rowcount == 0:
            conn.rollback()
            raise HTTPException(404, 'Клиент не найден')
...
@app.post('/api/termination-groups', dependencies=ADMIN_WRITE_AUTH)
...
@app.post('/api/terminators', dependencies=ADMIN_WRITE_AUTH)
def create_terminator(data: TerminatorIn):
    ...
    try:
        ...
        billing_cycle = db.normalize_billing_cycle(data.billing_cycle or db.default_billing_cycle_for_route(data.destination_name, data.prefix))
        conn.execute('BEGIN IMMEDIATE')
        if data.active:
...
@app.delete('/api/terminators/{tid}', dependencies=ADMIN_WRITE_AUTH)
...
@app.post('/api/client-rates', dependencies=ADMIN_WRITE_AUTH)
def create_client_rate(data: ClientRateIn):
    ...
    try:
        ...
        if exists is None:
            raise HTTPException(404, 'Клиент не найден')
        billing_cycle = db.normalize_billing_cycle(data.billing_cycle or db.default_billing_cycle_for_route(data.destination_name, data.prefix))
        cur = conn.execute('INSERT INTO client_rates (client_id, terminator_id, client_tech_prefix, prefix, destination_name, sell_rate_cents, billing_cycle) VALUES (?, ?, ?, ?, ?, ?, ?)', (data.client_id, data.terminator_id, data.client_tech_prefix, data.prefix, data.destination_name, data.sell_rate_cents, billing_cycle))
        conn.commit()
        return {'id': cur.lastrowid}
    finally:
        conn.close()
...
def list_client_rates():
    conn = db.get_conn()
    rows = conn.execute('SELECT cr.*, c.name AS client_name, t.name AS terminator_name, t.cost_rate_cents AS terminator_cost_rate_cents, t.tech_prefix AS terminator_tech_prefix, t.billing_cycle AS terminator_billing_cycle FROM client_rates cr JOIN clients c ON c.id = cr.client_id LEFT JOIN terminators t ON t.id = cr.terminator_id ORDER BY cr.client_id').fetchall()
    conn.close()
    return [dict(r) for r in rows]

@app.patch('/api/client-rates/{rid}', dependencies=ADMIN_WRITE_AUTH)
...
def _route_preview(conn, *, client, destination: str, terminator_id: Optional[int]=None):
    ...
    if rate_match is None:
        return {'ok': False, 'stage': 'client_rate', 'message': f'Нет тарифа клиента для {destination}', 'client': dict(client), 'input_destination': destination}
    rate, dial_destination, client_tech_prefix = rate_match
    route = db.get_terminator(conn, terminator_id or rate['terminator_id'])
    if route is None:
        route = db.match_active_terminator(conn, dial_destination)
    if route is None:
        return {'ok': False, 'stage': 'terminator', 'message': f'Нет терминатора для {dial_destination}', 'client': dict(client), 'client_rate': dict(rate), 'input_destination': destination, 'dial_destination': dial_destination, 'client_tech_prefix': client_tech_prefix}
    ...
    if not gateway_name and (not route_ip):
        return {'ok': False, 'stage': 'gateway', 'message': 'У терминатора не указан ни gateway, ни IP', 'client': dict(client), 'client_rate': dict(rate), 'terminator': dict(route), 'termination_group': dict(group) if group else None, 'input_destination': destination, 'dial_destination': dial_destination, 'client_tech_prefix': client_tech_prefix}
    provider_number = f"{route['tech_prefix'] or ''}{dial_destination}"
    if gateway_name:
        dial_string = f'sofia/gateway/{gateway_name}/{provider_number}'
    else:
        dial_string = f'sofia/external/{provider_number}@{route_ip}'
    return {'ok': True, 'stage': 'ready', 'message': 'Цепочка готова', 'client': dict(client), 'client_rate': dict(rate), 'terminator': dict(route), 'termination_group': dict(group) if group else None, 'input_destination': destination, 'client_tech_prefix': client_tech_prefix, 'dial_destination': dial_destination, 'gateway_name': gateway_name, 'route_ip': route_ip, 'terminator_tech_prefix': route['tech_prefix'] or '', 'provider_number': provider_number, 'dial_string': dial_string}
...
def _new_direction_preview(conn, *, client, route, destination: str, prefix: str, client_tech_prefix: str):
    ...
    route_ip = db.pick_ip(route_ips, destination)
    if not gateway_name and (not route_ip):
        return {'ok': False, 'stage': 'gateway', 'message': 'У терминатора не указан ни gateway, ни IP', 'client': dict(client), 'terminator': dict(route), 'termination_group': dict(group) if group else None, 'input_destination': destination, 'dial_destination': dial_destination, 'client_tech_prefix': client_tech_prefix}
    provider_number = f"{route['tech_prefix'] or ''}{dial_destination}"
    if gateway_name:
        dial_string = f'sofia/gateway/{gateway_name}/{provider_number}'
    else:
        dial_string = f'sofia/external/{provider_number}@{route_ip}'
    return {'ok': True, 'stage': 'ready', 'message': 'Цепочка будет готова после сохранения', 'client': dict(client), 'client_rate': None, 'terminator': dict(route), 'termination_group': dict(group) if group else None, 'input_destination': destination, 'client_tech_prefix': client_tech_prefix, 'dial_destination': dial_destination, 'gateway_name': gateway_name, 'route_ip': route_ip, 'terminator_tech_prefix': route['tech_prefix'] or '', 'provider_number': provider_number, 'dial_string': dial_string}

@app.post('/api/directions', dependencies=ADMIN_WRITE_AUTH)
def create_direction(data: DirectionIn):
    ...
    try:
        ...
        existing = conn.execute(
            'SELECT * FROM client_rates WHERE client_id = ? AND client_tech_prefix = ? AND prefix = ? AND terminator_id = ?',
            (data.client_id, data.client_tech_prefix, data.prefix, data.terminator_id),
        ).fetchone()
        billing_cycle = db.normalize_billing_cycle(data.billing_cycle or db.default_billing_cycle_for_route(data.destination_name, data.prefix))
        ...
        if existing is None:
            ...
            rate_id = existing['id']
            conn.execute('UPDATE client_rates SET destination_name = ?, sell_rate_cents = ?, billing_cycle = ? WHERE id = ?', (data.destination_name, data.sell_rate_cents, billing_cycle, rate_id))
        else:
            conn.rollback()
        ...
        conn.commit()
        return {'ok': True, 'created': created, 'client_rate_id': rate_id, 'billing_cycle': billing_cycle, 'preview': preview}
    except HTTPException:
        raise
    finally:
        conn.close()

@app.post('/api/route-check', dependencies=ADMIN_WRITE_AUTH)
def route_check(data: RouteCheckIn):
    ...
    try:
        ...
        if route is None:
            raise HTTPException(404, 'Терминатор не найден')
        return _new_direction_preview(conn, client=client, route=route, destination=destination, prefix=data.prefix, client_tech_prefix=data.client_tech_prefix)
    finally:
        conn.close()
...
def ensure_ops_client_route(data: OpsClientRouteIn):
    ...
    try:
        ...
        if route is None:
            ...
            route_billing_cycle = db.normalize_billing_cycle(data.billing_cycle or _row_value(source_route, 'billing_cycle', db.default_billing_cycle_for_route(data.destination_name, data.prefix)))
            cur = conn.execute('INSERT INTO terminators (name, gateway_group_id, ips, destination_name, prefix, gateway_name, tech_prefix, cost_rate_cents, billing_cycle, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1)', (f"{source_route['name']} {data.destination_name}", source_route['gateway_group_id'] if 'gateway_group_id' in source_route.keys() else None, source_route['ips'], data.destination_name, data.prefix, source_route['gateway_name'], source_route['tech_prefix'], cost_rate, route_billing_cycle))
            route = conn.execute('SELECT * FROM terminators WHERE id = ?', (cur.lastrowid,)).fetchone()
            created_terminator = True
        route_billing_cycle = db.normalize_billing_cycle(_row_value(route, 'billing_cycle', db.default_billing_cycle_for_route(data.destination_name, data.prefix)))
        rate_billing_cycle = db.normalize_billing_cycle(data.billing_cycle or db.default_billing_cycle_for_route(data.destination_name, data.prefix))
        sell_rate = data.sell_rate_cents
        if sell_rate is None:
            sell_rate = max(int(route['cost_rate_cents']) + 400, 1)
        existing_rate = conn.execute(
            'SELECT * FROM client_rates WHERE client_id = ? AND client_tech_prefix = ? AND prefix = ? AND terminator_id = ?',
            (client['id'], data.client_tech_prefix, data.prefix, route['id']),
        ).fetchone()
        if existing_rate is None:
            cur = conn.execute('INSERT INTO client_rates (client_id, terminator_id, client_tech_prefix, prefix, destination_name, sell_rate_cents, billing_cycle) VALUES (?, ?, ?, ?, ?, ?, ?)', (client['id'], route['id'], data.client_tech_prefix, data.prefix, data.destination_name, sell_rate, rate_billing_cycle))
            rate_id = cur.lastrowid
            created_rate = True
        else:
            rate_id = existing_rate['id']
            conn.execute('UPDATE client_rates SET destination_name = ?, sell_rate_cents = ?, billing_cycle = ? WHERE id = ?', (data.destination_name, sell_rate, rate_billing_cycle, rate_id))
            created_rate = False
        conn.commit()
        return {'ok': True, 'client_id': client['id'], 'terminator_id': route['id'], 'client_rate_id': rate_id, 'created_terminator': created_terminator, 'created_client_rate': created_rate, 'prefix': data.prefix, 'destination_name': data.destination_name, 'sell_rate_cents': sell_rate, 'cost_rate_cents': route['cost_rate_cents'], 'sell_billing_cycle': rate_billing_cycle, 'cost_billing_cycle': route_billing_cycle}
    except HTTPException:
        raise
    finally:
        conn.close()

def _legacy_dashboard_value(value):
    if value is None:
        return value
    return value / db.LEGACY_CENT_TO_MONEY_UNITS

def _legacy_dashboard_rows(rows, money_fields):
    converted = []
    for row in rows:
        item = dict(row)
        for field in money_fields:
            if field in item:
                item[field] = _legacy_dashboard_value(item[field])
        converted.append(item)
    return converted

def _dashboard_rows(rows):
    return [dict(r) for r in rows]
...
def dashboard_data(request: Request):
    ...
    if not _uses_current_money_scale(request):
        return {
            ...
            'summary': {
                'total_balance_cents': _legacy_dashboard_value(total_balance),
                'margin_today_cents': _legacy_dashboard_value(margin_today),
                'margin_month_cents': _legacy_dashboard_value(margin_month),
            },
        }
    return {'money_scale': db.MONEY_SCALE, 'clients': _dashboard_rows(clients), 'termination_groups': _dashboard_rows(groups), 'terminators': _dashboard_rows(terminators), 'client_rates': _dashboard_rows(client_rates), 'cdr': _dashboard_rows(cdr), 'sip_hits': _dashboard_rows(sip_hits), 'e164_directions': e164_directions, 'summary': {'total_balance_cents': total_balance, 'margin_today_cents': margin_today, 'margin_month_cents': margin_month}}
