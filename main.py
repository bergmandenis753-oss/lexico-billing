...
def finalize(data: FinalizeIn):
    ...
    try:
        ...
        conn.execute('INSERT INTO cdr (client_id, call_uuid, sip_ip, clid, destination, client_tech_prefix, dial_destination, provider_number, gateway_name, route_ip, terminator_id, terminator_name, terminator_destination_name, terminator_prefix, terminator_tech_prefix, hangup_cause, bridge_hangup_cause, result, billsec, sell_rate_cents, cost_rate_cents, charged_cents, margin_cents) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', (data.client_id, data.call_uuid, data.sip_ip, data.clid, data.destination, data.client_tech_prefix, data.dial_destination, data.provider_number, data.gateway_name, data.route_ip, data.terminator_id, data.terminator_name, data.terminator_destination_name, data.terminator_prefix, data.terminator_tech_prefix, data.hangup_cause, data.bridge_hangup_cause, data.result, data.billsec, data.sell_rate_cents, data.cost_rate_cents, charged, margin))
...
def create_terminator(data: TerminatorIn):
    ...
    try:
        ...
        cur = conn.execute('INSERT INTO terminators (name, gateway_group_id, ips, destination_name, prefix, gateway_name, tech_prefix, cost_rate_cents, active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', (data.name, data.gateway_group_id, data.ips, data.destination_name, data.prefix, data.gateway_name, data.tech_prefix, data.cost_rate_cents, int(data.active)))
        conn.commit()
        return {'id': cur.lastrowid}
    finally:
        conn.close()
...
def create_client_rate(data: ClientRateIn):
    ...
    try:
        ...
        if exists is None:
            raise HTTPException(404, 'Клиент не найден')
        cur = conn.execute('INSERT INTO client_rates (client_id, terminator_id, client_tech_prefix, prefix, destination_name, sell_rate_cents) VALUES (?, ?, ?, ?, ?, ?)', (data.client_id, data.terminator_id, data.client_tech_prefix, data.prefix, data.destination_name, data.sell_rate_cents))
        conn.commit()
        return {'id': cur.lastrowid}
    finally:
        conn.close()
...
def create_direction(data: DirectionIn):
    ...
    try:
        ...
        if existing is None:
            ...
            rate_id = existing['id']
            conn.execute('UPDATE client_rates SET terminator_id = ?, destination_name = ?, sell_rate_cents = ? WHERE id = ?', (data.terminator_id, data.destination_name, data.sell_rate_cents, rate_id))
        else:
            conn.rollback()
...
def ensure_ops_client_route(data: OpsClientRouteIn):
    ...
    try:
        ...
        if existing_rate is None:
            ...
            conn.execute('UPDATE client_rates SET terminator_id = ?, destination_name = ?, sell_rate_cents = ? WHERE id = ?', (route['id'], data.destination_name, sell_rate, rate_id))
            created_rate = False
        conn.commit()
        return {'ok': True, 'client_id': client['id'], 'terminator_id': route['id'], 'client_rate_id': rate_id, 'created_terminator': created_terminator, 'created_client_rate': created_rate, 'prefix': data.prefix, 'destination_name': data.destination_name, 'sell_rate_cents': sell_rate, 'cost_rate_cents': route['cost_rate_cents']}
    except HTTPException:
        raise
    finally:
        conn.close()
